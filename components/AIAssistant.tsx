  ...
  if (inline) {
    return (
      <div className="flex flex-col h-full bg-white relative">
        ...
        <form onSubmit={handleSubmit} className="p-4 bg-slate-50 border-t border-slate-200/50">
          ...
          <div className="relative group">
            <input 
              ...
              className="w-full bg-white border border-slate-200 rounded-xl py-2 pl-3 pr-9 text-xs font-semibold text-slate-900 placeholder:text-slate-400 focus:ring-2 focus:ring-green-500/15 focus:border-green-500 transition-all shadow-sm outline-none"
            />
            <button 
              type="submit"
              disabled={isProcessing}
    ...
    <>
      ...
      <div className={`fixed bottom-24 right-8 w-[440px] h-[82vh] max-h-[850px] bg-white border border-slate-200 rounded-[32px] shadow-[0_20px_60px_rgba(0,0,0,0.15)] z-40 transition-all duration-500 cubic-bezier(0.4, 0, 0.2, 1) transform ${isOpen ? 'translate-y-0 opacity-100 scale-100' : 'translate-y-10 opacity-0 scale-95 pointer-events-none'}`}>
        <div className="flex flex-col h-full">
          ...
          <form onSubmit={handleSubmit} className="p-6 bg-slate-50 border-t border-slate-200/50 rounded-b-[32px]">
            {/* Attached items indicators */}
            {attachedFile && (
            ...
            <div className="relative group">
              <input 
                ...
                className="w-full bg-white border border-slate-200 rounded-[18px] py-3 pl-5 pr-12 text-sm font-medium text-slate-900 placeholder:text-slate-400 focus:ring-4 focus:ring-green-500/10 focus:border-green-500 transition-all shadow-sm outline-none"
              />
              <button 
                type="submit"
                disabled={isProcessing}
